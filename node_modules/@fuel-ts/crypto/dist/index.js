...
function ripemd160(_data) {
  const data = (0, import_utils.arrayify)(_data, "data");
  return (0, import_ripemd160.ripemd160)(data);
}
...
function pbkdf2(_password, _salt, iterations, keylen, algo) {
  const password = (0, import_utils2.arrayify)(_password, "password");
  const salt = (0, import_utils2.arrayify)(_salt, "salt");
  return (0, import_utils2.hexlify)((0, import_crypto.pbkdf2Sync)(password, salt, iterations, keylen, algo));
}

// src/node/randomBytes.ts
var import_crypto2 = __toESM(require("crypto"));
var randomBytes = (length) => {
  const randomValues = Uint8Array.from(import_crypto2.default.randomBytes(length));
  return randomValues;
};
...
var keyFromPassword = (password, saltBuffer) => {
  ...
  return (0, import_utils3.arrayify)(key);
};
var encrypt = async (password, data) => {
  ...
  const cipher = await import_crypto3.default.createCipheriv(ALGORITHM, secret, iv);
...
var decrypt = async (password, keystore) => {
  ...
  const decipher = await import_crypto3.default.createDecipheriv(ALGORITHM, secret, iv);
...
function computeHmac(algorithm, _key, _data) {
  const key = (0, import_utils4.arrayify)(_key, "key");
  const data = (0, import_utils4.arrayify)(_data, "data");
  return (0, import_utils4.hexlify)((0, import_crypto5.createHmac)(algorithm, key).update(data).digest());
}
